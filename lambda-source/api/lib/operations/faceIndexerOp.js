...
class FaceIndexerOp extends BaseOp {
  ...
  async onGetFacesByCollection() {
    ...
    if (!collectionId || !validateFaceCollectionId(collectionId)) {
  ...
  async onIndexFaceFromBlob() {
    ...
    if (!validateFaceCollectionId(collectionId)) {
      throw new M2CException('invalid collectionId');
    }
    if (!validateCharacterSet(celeb)) {
  ...
  async onDeleteFace() {
    ...
    if (!_collectionId || !validateFaceCollectionId(_collectionId)) {
      throw new M2CException('invalid collection id');
    }
