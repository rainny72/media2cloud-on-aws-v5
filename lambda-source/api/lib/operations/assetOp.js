...
class AssetOp extends BaseOp {
  async onGET() {
    ...
    if (token && !validateBase64JsonToken(token)) {
  ...
  async startIngestWorkflow(params) {
    ...
    input.destination = {
      ...
      prefix: makeSafeOutputPrefix(input.uuid, key),
      ...input.destination,
    };

    // execution friendly name
