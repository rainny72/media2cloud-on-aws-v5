...
const StateRunTitanEmbeddings = require('./states/run-titan-embeddings');
const StatePrepareFrameAnalysisIterators = require('./states/prepare-frame-analysis-iterators');
const StateMapIterationsCompleted = require('./states/map-iterations-completed');
...
function parseEvent(event, context) {
  ...
  parallelStateOutputs.forEach((output) => {
    ...
    output.forEach((out) => {
      if (out.data.iterators.length === 0) {
        // populate framesegmenation data even if there is no rekog job.
        for (const name in out.data) {
          if (name !== 'iterators') {
            iterators.push({
              [name]: out.data[name],
            });
  ...
  try {
    ...
    switch (operation) {
      ...
      case StateData.States.CreateSceneEvents:
        ...
        break;
      case StateData.States.RunTitanEmbeddings:
        instance = new StateRunTitanEmbeddings(stateData);
        break;
    ...
    if (!instance) {
      if (StatePrepareFrameAnalysisIterators.opSupported(operation)) {
        instance = new StatePrepareFrameAnalysisIterators(stateData);
      } else if (StateMapIterationsCompleted.opSupported(operation)) {
        instance = new StateMapIterationsCompleted(stateData);
      }
    }
