...
const MERGE_SHORT_SCENES = false;
...
class StateCreateSceneEvents {
  ...
  async process() {
    ...
    scenes.forEach((scene, idx) => {
      ...
      const sceneStart = toTimecode(enumFPS, scene.frameStart, dropFrame)[0];
      const sceneEnd = toTimecode(enumFPS, scene.frameEnd, dropFrame)[0];
      console.log(`[scene#${String(scene.sceneNo).padStart(3, '0')}]: ${String(scene.shotStart).padStart(4, ' ')} - ${String(scene.shotEnd).padStart(4, ' ')}, ${sceneStart} -> ${sceneEnd} (${_toHHMMSS(scene.duration, true)}) [${scene.technicalCueType}] [FadeInOut = ${!!scene.fadeInBlack}, ${!!scene.fadeOutBlack}]`);
    });
...
function _computeShotSimilarity(framesegmentation, range) {
  ...
  framesA.forEach((frameA) => {
    framesB.forEach((frameB) => {
      similarities.push(
        cosim(frameA.embeddings, frameB.embeddings)
      );
    });
  });
