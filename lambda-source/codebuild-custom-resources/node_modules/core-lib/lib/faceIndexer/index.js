...
class FaceIndexer {
  ...
  async registerFace(faceId, fields) {
    ...
    return db.update(
      ...
      this.itemsCached[faceId] = { ..._fields, faceId };
  ...
  async deleteFaces(collectionId, faceIds = []) {
    if (!collectionId || faceIds.length === 0) {
      return undefined;
    }

    const _faceIds = [...new Set(faceIds)];
    return await _deleteFacesFromCollection(collectionId, _faceIds);
  }

  async getByCeleb(celeb) {
    return await _queryCeleb(celeb);
  }
