...
exports.CheckBucketRegion = async (event, context) => {
  const x0 = new X0(event, context);
  ...
  try {
    ...
    if (!((event.ResourceProperties || {}).Data || {}).Bucket) {
      x0.storeResponseData('Region', '');
      x0.storeResponseData('Status', 'SKIPPED');
      return x0.responseData;
    }

    const bucket = event.ResourceProperties.Data.Bucket;
    ...
    const region = await s3Client.send(command)
      .then((res) =>
        res.LocationConstraint || 'us-east-1');

    x0.storeResponseData('Region', region);
    x0.storeResponseData('Status', 'SUCCESS');
